...
class TestCustomSet(unittest.TestCase):
    ...
    def testAddRemove(self):
        ...
        for i in range(n):
            ...
            self.assertEqual(bs_error, cs_error,
                             msg=f"built-in set error is {bs_error} but custom set error is {cs_error} when removing {i}")
    ...
    def testSpeed(self):
        ...
        self.assertLess(t_add_remove, t_max,
                        f"adding {n} items then removing {n} items took {t_add_remove:.2f} ms, which is slower than the required {t_max} ms")
